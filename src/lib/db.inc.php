 ...
 * Email: rvercesi@t2ws.biz
 * Filename: _db.inc.php
 * Description: Abstract Database Class
 * Notes: Based on dbFacile
*/
...
abstract class dbClass {
	...
	public static function open($type, $database, $user = '', $password = '', $host = 'localhost') {
		...
		switch($type) {
			...
			case 'sqlite':
				if(is_resource($database)) {
					$o = new dbClass_sqlite($database);
				}
				if(is_string($database)) {
					$o = new dbClass_sqlite();
					$o->_open($database);
				}
				return $o;
				break;
	...
	public function execute($sql, $parameters = array()) {
		...
		if(!$this->result && (error_reporting() & 1))
			trigger_error('dbClass - Error in query: ' . $this->query . ' <br/> ' . $this->_error(), E_USER_WARNING);
	...
	public function insert($data, $table) {
		...
		if(is_string($data) && is_array($table)) {
			...
			$data = $table;
			$table = $tmp;
			//trigger_error('dbClass - Parameters passed to insert() were in reverse order, but it has been allowed', E_USER_NOTICE);
		}
	...
	public function update($data, $table, $where = null, $parameters = array()) {
		...
		if(is_string($data) && is_array($table)) {
			...
			$data = $table;
			$table = $tmp;
			trigger_error('dbClass - The first two parameters passed to update() were in reverse order, but it has been allowed', E_USER_NOTICE);
		}
		if($this->filterInvalidFields)
			$data = $this->filterFields($data, $table);
		// wrap quotes around values that need them
	...
	public function fetch($sql, $parameters = array()) {
        ...
        public function getNumRows($sql, $parameters = array()){
                //$this->execute($sql, $parameters);
				//$this->res = $this->execute($sql, $parameters);
	...
	private function getTableInfo($table) {
		...
		if($rows) {
			...
			die('dbClass - Table "' . $table . '" does not exist');
	}
...
class dbClass_mssql extends dbClass {
...
class dbClass_mysql extends dbClass {
	private $database;
...
/*
class dbClass_mysqli extends dbClass {
	public function __construct($handle = null) {
		parent::__construct();
...
/*
class dbClass_postgresql extends dbClass {
	public function __construct($handle = null) {
...
class dbClass_sqlite extends dbClass {
	public function beginTransaction() {
	...
	protected function _fetch() {
		return new dbClass_sqlite_result($this->result);
	}
...
/*
class dbClass_sqlite_result implements Iterator {
	private $result;
...
class dbClass_pdo_postgresql extends dbClass_pdo {
	public function __construct($handle = null) {
		parent::__construct($handle);
		$this->schemaNameField = 'column_name';
		$this->schemaTypeField = 'data_type';
...
class dbClass_pdo_sqlite extends dbClass_pdo {
	public function __construct($handle = null) {
		parent::__construct($handle);
...
class dbClass_pdo_sqlite2 extends dbClass_pdo {
	public function __construct($handle = null) {
		parent::__construct($handle);
		$this->schemaNameField = 'name';
		$this->schemaTypeField = 'type';
...
abstract class dbClass_pdo extends dbClass {
